...
export function sceneVisualLine(visual: SceneVisual, vehicle: 'car' | 'bike' = 'car'): string | null {
  switch (visual.kind) {
    ...
    case 'matched':
      return `For the ${visual.topic}, take how it looks from the supplied reference image ${visual.photo.filename} (${visual.photo.label}) — on the real ${vehicle} in the scene, never as the photo itself.${visual.topic === 'interior' ? ' Film it from a seat or through an open door; anyone in shot sits in a seat or stands outside at the door.' : ''}`;
    case 'generic':
      return `No supplied photo shows the ${visual.topic}. Render a realistic, generic ${visual.topic} that suits this ${vehicle} — do not reshape a reference photo of another part to fake it.${visual.topic === 'interior' ? ' Film it from a seat or through an open door; anyone in shot sits in a seat or stands outside at the door.' : ''}`;
    default:
      return null;
  }
}
